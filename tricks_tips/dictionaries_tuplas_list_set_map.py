...
All examples are from the internet with some variations,
...
# You can use the filter method to filter a list of values, in this case, the values
# "\None" or empty. Be careful with the zero value.
print(f"{list(filter(None, n_values_in_list))=}")

# use An assignment expression (sometimes also called a “named expression” or “walrus”)
...
while (command := input("Ingresa quit para salir: ")) != "quit":
    print(f"Ingresaste {command}")
...
# example 2: use slicing from beginning to end using negative step
print(f"{users[::-1]=}")
...
# ----------------------------------------------------------------------------------------
# or use this way to Flatten a 2D list
list_to_flatten: list = [[1, 3, 4], [7, 5, 2], [8, 9, 0]]
flatten_list_result = [i for j in list_to_flatten for i in j]
print(f"{flatten_list_result=}")
...
# if you want to get all the values & keys in a dictionary
# you can use the items method in a loop.
for key, value in user.items():
    print(f"--> {key}: {value}")

# you can use the popitem method to delete the last item in the dictionary,
# and save the key and value in a variable.
...
def greeting(key_user):
    """
    Greet a user with a given key, or "there" if the key is not found.

    Parameters
    ----------
    key_user : str
        Key of the user to be greeted.
...
# sorcery skip: dict-assign-update-to-union, remove-redundant-constructor-in-dict-union
...
def find_price(item: str) -> str:
    """
    Finds the price of a given item in the products_and_price dictionary.

    Args:
        item (str): The name of the item to find the price for.

    Returns:
        str: A string indicating the price of the item or "not found" if the item is not in the dictionary.
    """
...
# You can use the map and the list with two iterables to obtain a list with the result of
# numbers raised to the power n of another list.
print(f"{list(map(pow, numbers_a_bases, numbers_b_exponents))=}")

# If you need remove duplicates from a list, you can use dictionaries with loops and preserve
...
# If you need to remove duplicates from a list, you can use the fromkeys and list method to
# generate a new list, but you lose the order of the elements.
...
# -------------------------------------------
# You can use the zip function to make N-tuples from N arguments, so it can be used with
# *args to transpose a list of lists.
...
# ------------------------------------------------------------------
# Sorting a list of dictionaries, we can do it using two different,
# yet similar approaches. We can simply use the in-built sorted() or sort()
# function which works on lists using a lambda function inside it which
# sorts the dictionaries based on their ‘id’ key
